...
- 3章1節 タスクを分割する — レビュー反映 (branch: fix/ch03-01)
...
- [ ] 出典のない経験的主張を書かせない手順を追加する — `.claude/commands/draft.md`（3回ルール発火: fact「経験的な主張に出典がない」3件）
- [ ] 反論の処理を執筆時に点検する手順を追加する — `.claude/commands/draft.md`（3回ルール発火: logic「反論への言及がない」3件）
- [ ] 段落の第1文が前置きや指示語にならないよう規約を補強する — `foundation/style-guide.md`（3回ルール発火: paragraph「段落の第1文が前置きや指示語で内容を含まない」3件）
- [ ] 主語と述語の距離を執筆時に点検する手順を追加する — `.claude/commands/draft.md`（3回ルール発火: tech-writing「一文が長く主語と述語が離れている」3件）
